 ...
 */
public abstract class StreamAggregation<K, EV, S extends Serializable, T> implements Serializable {
     ...
     * operates edge-wise
     */
    private final SketchFold<K, EV, S> updateFun;
    ...
    protected StreamAggregation(SketchFold<K, EV, S> updateFun, ReduceFunction<S> combineFun, MapFunction<S, T> transform, S initialValue, boolean transientState) {
    ...
    public abstract DataStream<T> run(DataStream<Tuple2<K, EV>> edgeStream);
    ...
    public SketchFold<K, EV, S> getUpdateFun() {
        return updateFun;
    }
    ...
    protected FlatMapFunction<S, S> getAggregator(final DataStream<Tuple2<K, EV>> edgeStream) {
