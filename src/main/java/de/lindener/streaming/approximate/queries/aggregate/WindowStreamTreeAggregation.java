...
package de.lindener.streaming.approximate.queries.aggregate;
...
/**
 * WIP Graph Tree Aggregation on Parallel Time Window
 *  
 * 
 */
public class WindowStreamTreeAggregation<K, EV, S extends Serializable, T> extends WindowStreamAggregation<K, EV, S, T> {

	private static final long serialVersionUID = 1L;
	private int degree;
	

	public WindowStreamTreeAggregation(SketchFold<K, EV, S> updateFun, ReduceFunction<S> combineFun, MapFunction<S, T> transformFun, S initialVal, long timeMillis, boolean transientState, int degree) {
		super(updateFun, combineFun, transformFun, initialVal, timeMillis, transientState);
		this.degree = degree;
	}

	public WindowStreamTreeAggregation(SketchFold<K, EV, S> updateFun, ReduceFunction<S> combineFun, S initialVal, long timeMillis, boolean transientState, int degree) {
		this(updateFun, combineFun, null, initialVal, timeMillis, transientState, degree);
	}

	public WindowStreamTreeAggregation(SketchFold<K, EV, S> updateFun, ReduceFunction<S> combineFun, S initialVal, long timeMillis, boolean transientState) {
	...
	public DataStream<T> run(final DataStream<Tuple2<K, EV>> edgeStream) {
		TypeInformation<Tuple2<Integer, Tuple2<K, EV>>> basicTypeInfo = new TupleTypeInfo<>(BasicTypeInfo.INT_TYPE_INFO, edgeStream.getType());

		TupleTypeInfo edgeTypeInfo = (TupleTypeInfo) edgeStream.getType();
		TypeInformation<S> partialAggType = TypeExtractor.createTypeInfo(SketchFold.class, getUpdateFun().getClass(), 2, edgeTypeInfo.getTypeAt(0), edgeTypeInfo.getTypeAt(1));
